...
class UniversityTimeTable(models.Model):
    ...
    sat_timetable_ids = fields.One2many('timetable.schedule.line',
                                        ...
                                        domain=[('week_day', '=', '5')])
    sun_timetable_ids = fields.One2many('timetable.schedule.line',
                                        'timetable_id',
                                        help="Scheduled line of Sunday",
                                        domain=[('week_day', '=', '6')])
    uwa_timetable_ids = fields.One2many('uwa.timetable.schedule.line',
                                        'timetable_id',
                                        help="Scheduled line of UWA")
    ...
    def get_lines_time_table(self):
        """This method returns the scheduled lines of the timetable"""
        # Get the scheduled lines of the timetable ordered by time_from
        result = []
        for day in range(7):
            lines = self.env['timetable.schedule.line'].search(
                [('timetable_id', '=', self.id), ('week_day', '=', str(day))],
                order='time_from')
            result.append(lines)
        return result